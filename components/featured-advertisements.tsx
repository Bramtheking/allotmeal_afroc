  ...
  useEffect(() => {
    const fetchAdvertisements = async () => {
      try {
        ...
        const q = query(collection(db, "advertisements"), where("status", "==", "active"), limit(10))
        ...
        querySnapshot.forEach((doc) => {
          ...
          // Filter out expired ads on client-side as well
          // If no endDate exists (old ads), show them
          // If endDate exists, only show if it's in the future
          if (!adData.endDate || adData.endDate > now) {
            ads.push({ id: doc.id, ...adData })
            console.log(`✓ Including ad: ${adData.title}`)
          } else {
